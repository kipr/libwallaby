...
try_again:
	...
	ret = os_snprintf(ctrl->local.sun_path, sizeof(ctrl->local.sun_path),
					  CONFIG_CTRL_IFACE_CLIENT_DIR "/" CONFIG_CTRL_IFACE_CLIENT_PREFIX "%d-%d",
					  (int)getpid(), counter);
	...
	else
	{
		...
		printf("res: %d\n", res);
		printf("sizeof(ctrl->dest.sun_path): %d\n", sizeof(ctrl->dest.sun_path));
	...
	printf("connect(ctrl->s, (struct sockaddr *)&ctrl->dest, sizeof(ctrl->dest)): %d\n", connect(ctrl->s, (struct sockaddr *)&ctrl->dest, sizeof(ctrl->dest)));
	if(errno != NULL){
		printf("errno: %d\n", errno);
	}
